...
function InfoSection({ title, items }) {
  return (
    <div className="contentSection mb-8">
      <h3 className="px-4 py-2 w-fit border-[#FF97DB] border-4 rounded-[50px] bg-transparent text-[#212121] font-bold leading-none uppercase mb-4"
      ...
      {items.map((item, idx) => (
        <div key={idx} className="mb-2">
          {item.label && (
            <h3 className="font-semibold text-lg md:text-xl" style={{ marginBottom: 2 }}>{item.label}</h3>
          )}
...
export default function About() {
  ...
  const desktopContent = (
    <section className="h-auto w-[100vw] pb-[1vh] bg-white top-0 flex flex-col">
      ...
      <div className={`content flex ${windowWidth > 1200 ? "flex-row" : "flex-col"}`}>
        <div className="textArea flex flex-col gap-[2vw] ml-[5vw]">
          <div className="flex flex-col md:flex-row gap-[3vw]">
            <section className="w-full md:w-1/2"><InfoSection title="Technical" items={technical} /></section>
            <section className="w-full md:w-1/2"><InfoSection title="Design" items={design} /></section>
          </div>
          <div className="flex flex-col md:flex-row gap-[3vw]">
